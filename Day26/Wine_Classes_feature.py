...
class classification_analysis_feature:
    def __init__(self, data, target_column, test_size=0.2, random_state=42, poly_degree=2, scaler_type='standard', models=None, stratify=None, idx=0.1):
        ...
        # 특성 중요도 계산을 위한 모델 학습 
        self.feature_importance_model = DecisionTreeClassifier(random_state=random_state)
        self.feature_importance_model.fit(self.train_input, self.train_target)
        self.feature_importances = self.feature_importance_model.feature_importances_

        # 중요도가 낮은 특성들 제거
        important_features = self.feature_importances > idx
        self.train_input = self.train_input[:, important_features]
        self.test_input = self.test_input[:, important_features]
